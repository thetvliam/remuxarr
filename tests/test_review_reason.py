...
Four mutants, all confirmed surviving the full suite before this file
existed:
...
Six more scoped the two bulk resolvers, which read this column to decide
which items a library-wide setting could sweep up. They are gone: one card
now answers one question for every file it covers, which is what those
buttons were a blunt stand-in for, and their tests went with them.
